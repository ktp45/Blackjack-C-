...
void play(short decknum, vector<Cards> &deck, double &bet, double &money, double &pairbet, double &warbet) // function which leads the game
{
	...
	money -= bet;
	money -= pairbet;
	money -= warbet; // we put the bets on the table
	while (!winner) // we play till there is a winner
	{
		if (deck.empty() || deck.size() <= 38) // if  there is a deck made by 38 aces player will have 21 and the dealer 17, that is the worst case scenario
		{
		...
		if (pairbet != 0) // same as warbet
		{
			if (IsPair(your_hand))
			{
				...
				if (your_hand[0].GetSuit() == your_hand[1].GetSuit())
					pairbet *= 30;
				else pairbet *= 7;
				money += pairbet;
				pairbet = 0;
