...
{
	 if (c.GetSuit() == 0)
	 	return out << c.GetName()<<"-C"; // C for Clubs, D for Diamonds, H for Hearts and S for Spades
 	 if (c.GetSuit() == 1)
	 	return out << c.GetName()<<"-D";
   if (c.GetSuit() == 2)
	 	return out << c.GetName()<<"-H";
   if (c.GetSuit() == 3)
		return out << c.GetName()<<"-S";
}
...
short Calcscore(vector<Cards> hand) // function that calculates the Blackjack score of a hand
{
	...
	for (int i = 0; i < len; i++)
	{
		if (hand[i].GetPower() != 1)
			score += hand[i].GetPower(); // ace is 11 or 1 if  the score is above 21
		else
		{
			numaces++;
			score += 11;
...
{
	if (hand[0].GetName() == hand[1].GetName())
		return 1;
	else return 0;
}
